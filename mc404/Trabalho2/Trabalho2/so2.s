...
SET_STACK_POSITIONS:
    ...
    .set STACK_USER, 0x77803200
    .set STACK_SUPERVISOR, 0x77803300
    .set STACK_IRQ, 0x77803400
...
SET_TZIC:
    ...
    msr CPSR_c, #0xD3           @Seta o modo SUPERVISOR
...
VERIFICA_ALARMES:
    ...
    loop_verificacoes:
      ...
      msr CPSR_c, #0xD0               @Muda para o modo USUARIO
...
VERIFICA_CALLBACKS:
      ...
      mov r10, #0                         @Coloca em r3 o contador de callbacks verificadas
      ...
      loop_verifica_callbacks:
        ...
        msr CPSR_c, #0xDF                 @Muda para o modo SYSTEM para poder empilhar o id
...
SET_TIME:
    push {r1-r12, lr}	@Salva todos os registradores
    msr CPSR_c, #0xDF	@Muda para modo system
    ldr r0, [sp]	@Carrega em r0 o parametro que é o time a ser setado
    msr CPSR_c, #0xD3	@Ativa modo supervisor
...
GET_TIME:
    push {r1-r12, lr}	@Salva todos os registradores
    msr CPSR_c, #0xDF	@ativa modo system
