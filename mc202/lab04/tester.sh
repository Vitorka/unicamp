...
function fileNotFound {
	...
	echo "Lembrando que os arquivos devem ser nomeado: main.c, lab04.c, lab04.h e devem "
...
if [[ ! -f "main.c" ]]; then
	fileNotFound
	exit
else if [[ ! -f "lab04.h" ]]; then
	fileNotFound
	exit
else if [[ ! -f "lab04.c" ]]; then
	fileNotFound
	exit
fi
fi
