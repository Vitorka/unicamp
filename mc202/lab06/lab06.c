...
{   
    ...
    if(elemento == no_atual->item)
    {
        ...
        {
            no_maior = remove_busca(no_atual->esq);
            no_maior_pai = no_maior->pai;
            
            /*Verifica se o no que ira trocar, eh filho direto do no retirado*/
            if(no_atual == no_maior_pai)
            {
                /*Caso o no que ira substituir o no que sera retirado, possuir 
                filho, liga-o ao avo*/
                if(no_maior->esq != NULL)
                {
                    no_maior_pai->esq = no_maior->esq;
                    no_maior->esq->pai = no_maior_pai;
                }
                /*Caso nao possua filho, faz o avo apontar para NULL*/ 
                else
                {
                    no_maior_pai->esq = NULL;
                }
                
                no_atual->item = no_maior->item;
                free(no_maior);
            }
            ...
            else
            {
                ...
                if(no_maior->esq != NULL)
                {
                    no_maior_pai->dir = no_maior->esq;
                    no_maior->esq->pai = no_maior_pai;
                }
                /*Caso nao possua filho, faz o avo apontar para NULL*/ 
                else
                {
                    no_maior_pai->dir = NULL;   
                }
            ...
            return afunilamento(no_maior_pai);
        }
        ...
        else
        {
            ...
            if(no_atual->esq != NULL)
            {
                ...
                if(no_pai != NULL)
                {
                    ...
                    return afunilamento(no_pai);
                }
                
                /*Caso o no a ser retirado seja raiz*/
                no_auxiliar = no_atual->esq;
                free(no_atual);
