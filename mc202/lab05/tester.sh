...
function fileNotFound {
	...
	echo "Lembrando que os arquivos devem ser nomeados: lab05.c, tree.c, tree.h e devem "
...
if [[ ! -f "tree.c" ]]; then
	fileNotFound
	exit
else if [[ ! -f "tree.h" ]]; then
	fileNotFound
	exit
else if [[ ! -f "lab05.c" ]]; then
	fileNotFound
	exit
fi
fi
